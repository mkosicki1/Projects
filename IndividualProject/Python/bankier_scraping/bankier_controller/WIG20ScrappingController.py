...
class WIG20_ScrappingController:
    def __init__(self):
        # wykonanie żądania metodą GET na adres url -> obiekt strony
        wig20_page = requests.get("https://www.bankier.pl/inwestowanie/profile/quote.html?symbol=WIG20")
        # na zwróconym obiekcie strony parsujemy do formatu html
        self.wig20_html = BeautifulSoup(wig20_page.content, 'html.parser')
        # połącznie z DB
        db = toExport()
    ...
    def insertDataIntoDB(self,results):
        for result in results:
            self.cursor.execute("INSERT INTO WIG_20 VALUES (default, %s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    def deleteDataFromWIG20(self):
        self.cursor.execute("DELETE FROM WIG_20")
        self.connection.commit()


    def getContentWIG20(self):
...
wsc = WIG20_ScrappingController()
results = wsc.getContentWIG20()
