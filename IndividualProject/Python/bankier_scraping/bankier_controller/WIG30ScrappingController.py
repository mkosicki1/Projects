...
class WIG30_ScrappingController:
    def __init__(self):
        # wykonanie żądania metodą GET na adres url -> obiekt strony
        wig30_page = requests.get("https://www.bankier.pl/inwestowanie/profile/quote.html?symbol=WIG30")
        # na zwróconym obiekcie strony parsujemy do formatu html
        self.wig30_html = BeautifulSoup(wig30_page.content, 'html.parser')
        # połącznie z DB
        db = toExport()
    ...
    def insertDataIntoDB(self,results):
        for result in results:
            self.cursor.execute("INSERT INTO WIG_30 VALUES (default, %s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    def deleteDataFromWIG30(self):
        self.cursor.execute("DELETE FROM WIG_20")
        self.connection.commit()


    def getContentWIG30(self):
...
wsc = WIG30_ScrappingController()
results = wsc.getContentWIG30()
